...
public class CreateLaptop extends JDialog {
	...
	public boolean valid() {
		...
		int phanLoai = cboPhanLoai.getSelectedIndex();
		int dongLaptop = cboDongLaptop.getSelectedIndex();
		String namSanXuat = txtNamSanXuat.getText();
		
		if (maLaptop == null || maLaptop.trim().isEmpty() || tenLaptop == null || tenLaptop.trim().isEmpty() || phanLoai == -1 || dongLaptop == -1 || namSanXuat == null || namSanXuat.trim().isEmpty()) {
			throw new NullPointerException();
		}
		
		try {
			int nsx = Integer.parseInt(namSanXuat);
