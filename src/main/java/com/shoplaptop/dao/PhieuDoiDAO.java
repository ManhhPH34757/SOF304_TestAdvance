...
public class PhieuDoiDAO implements ShopLaptop365DAO<PhieuDoi, String>{
	...
	String InsertPhieuDoi_SQL = "INSERT dbo.PhieuDoi(MaPhieuDoi,MaKH,MaNV,NgayTao,HinhThucVanChuyen,HinhThucThanhToan,TongTien)VALUES( ?,?,?,?,?,?,?)";
	String DeletePhieuDoi_SQL = "DELETE FROM dbo.PhieuDoi WHERE MaPhieuDoi = ?";
	String SelectPhieuDoiAll_SQL = "SELECT dbo.PhieuDoi.ID, dbo.PhieuDoi.MaPhieuDoi, dbo.PhieuDoi.MaNV,dbo.PhieuDoi.MaKH, dbo.PhieuDoi.MaNV,dbo.PhieuDoi.NgayTao,\r\n"
			+ "	dbo.PhieuDoi.HinhThucVanChuyen AS 'ID_HinhThucVanChuyen',HinhThucVanChuyen.HinhThuc AS 'HinhThucVanChuyen', \r\n"
			+ "	dbo.PhieuDoi.HinhThucThanhToan AS 'ID_HinhThucThanhToan',HinhThucThanhToan.HinhThuc AS 'HinhThucThanhToan',\r\n"
			+ "	dbo.PhieuDoi.TongTien\r\n"
			+ "FROM dbo.PhieuDoi JOIN dbo.HinhThucVanChuyen ON HinhThucVanChuyen.ID = PhieuDoi.HinhThucVanChuyen\r\n"
			+ "JOIN dbo.HinhThucThanhToan ON HinhThucThanhToan.ID = PhieuDoi.HinhThucThanhToan";
	...
	public String insert(PhieuDoi phieuDoi) {
		try {
			XJdbc.update(InsertPhieuDoi_SQL, phieuDoi.getMaPhieuDoi(),phieuDoi.getMaKH(),phieuDoi.getMaNV(),phieuDoi.getNgayTao(),phieuDoi.getID_HinhThucVanChuyen(),phieuDoi.getID_HinhThucThanhToan(),phieuDoi.getTongTien());
	...
	public List<PhieuDoi> selectBySQL(String sql, Object... args) {
		...
		try {
			...
			while(rs.next()) {
				...
				phieuDoi.setID_HinhThucVanChuyen(rs.getInt("ID_HinhThucVanChuyen"));
				phieuDoi.setHinhThucVanChuyen(rs.getString("HinhThucVanChuyen"));
				phieuDoi.setID_HinhThucThanhToan(rs.getInt("ID_HinhThucThanhToan"));
				phieuDoi.setHinhThucVanChuyen(rs.getString("HinhThucThanhToan"));
				phieuDoi.setTongTien(rs.getBigDecimal("TongTien"));
				list.add(phieuDoi);				
			}
